                  ...
                  {lesson.student_ids && lesson.student_ids.length > 0 && lesson.student_ids.length < students.length && (
                      <div className="flex -space-x-2 mt-2" title={lesson.student_ids.map(id => studentMap.get(id)?.name).join(', ')}>
                          {lesson.student_ids.slice(0,3).map(id => {
                              ...
                              return student?.avatar_url ? <img key={id} src={student.avatar_url} alt={student.name} className="w-5 h-5 rounded-full ring-2 ring-card"/> : <UserCircleIcon key={id} className="w-5 h-5 rounded-full ring-2 ring-card bg-card text-muted-foreground"/>
                          })}
